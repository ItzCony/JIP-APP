...
ZAKLADNI_PRAVA = {
    ...
    'sankce_tiket_unibrands': {'kategorie': 'Modul Sankce', 'podskupina': 'Nákupčí (tikety)', 'nazev': 'Tikety – Unibrands', 'popis': 'Vidí pouze dlaždici „Nedodávky dod. k vyjádření" a smí tam zapisovat do sloupce „Vyjádření nákupčího" u dodavatele UNIBRANDS. Žádné jiné sestavy ani tikety.', 'ikona': 'edit_note'},
    ...
    'cenopripad_zadatel_nakup':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Případy (nákup/obchod)', 'nazev': 'Žadatel nákup',  'popis': 'Podává žádosti o kontrolu letákových cen (dlaždice leták/sklad6/mimoleták/paima) a vidí jen své tikety a jejich stavy. Nevidí OP ani marže v %. KONTROLA CEN OVOZEL: vkládá soubory „OZ změny PC“ ke kontrole a vidí jen své kontroly; nenahrává sestavu Sklad 6, neexportuje ani nemaže. NC OVOZEL: zakládá žádosti o změnu nákupní ceny (import cen / delist), vidí jen své a opravuje ty vrácené.', 'ikona': 'sell'},
    'cenopripad_zadatel_obchod': {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Případy (nákup/obchod)', 'nazev': 'Žadatel obchod', 'popis': 'Podává žádosti o kontrolu individuálních cen (dlaždice porovnání) a vidí jen své tikety a jejich stavy. Nevidí OP ani marže v %.', 'ikona': 'sell'},
    'cenopripad_office_nakup':   {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Případy (nákup/obchod)', 'nazev': 'Office nákup',   'popis': 'Uzavírá a dotahuje schválené nákupní případy do nastavení a může je stornovat s uvedením důvodu. Rozhoduje se pouze podle verdiktu aplikace — NEvidí OP ani marže v %. KONTROLA CEN OVOZEL: nahrává týdenní sestavu Sklad 6, zadává kontroly, vidí všechny a exportuje kontrolní sestavu; chodí mu e-mail s výsledkem každé kontroly (mazat případy smí až Správce). NC OVOZEL: vidí všechny žádosti o změnu nákupní ceny, označuje je jako zpracované, vrací k opravě s důvodem a exportuje soubor pro import do skladu 6 (mazat smí až Správce).', 'ikona': 'task_alt'},
    'cenopripad_office_obchod':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Případy (nákup/obchod)', 'nazev': 'Office obchod',  'popis': 'Uzavírá a dotahuje schválené obchodní případy (porovnání) do nastavení a může je stornovat s uvedením důvodu. Rozhoduje se pouze podle verdiktu — NEvidí OP ani marže v %.', 'ikona': 'task_alt'},
    'cenopripad_office_obchod_ovozel': {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Kontrola cen OVOZEL', 'nazev': 'Office obchod – OVOZEL', 'popis': 'MIMOŘÁDNÉ PRÁVO pro Office obchod: vstup do obou dlaždic OVOZEL (Kontrola cen OVOZEL i NC OVOZEL) se stejným rozsahem jako Office nákup — nahrává sestavu Sklad 6, zadává kontroly, vidí všechny případy, exportuje, zpracovává žádosti o změnu NC a vrací je k opravě; chodí mu stejné e-maily a notifikace (mazat smí až Správce). Ostatní nákupní dlaždice modulu NEodemyká — držitel dál vidí jen IND ceny a OVOZEL.', 'ikona': 'task_alt'},
    'cenopripad_zadatel_letaky': {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Kontrolní data letáků', 'nazev': 'Žadatel – Letáková kontrola', 'popis': 'LETÁKOVÁ KONTROLA — Nahrává kontrolní sestavy letáků (dlaždice „Kontrolní data letáků“, kanály VO/MO/SP), vidí jen své nahrané sestavy a jejich stav a komunikuje v komentářích.', 'ikona': 'sell'},
    'cenopripad_office_letaky':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Kontrolní data letáků', 'nazev': 'Office – Letáková kontrola', 'popis': 'LETÁKOVÁ KONTROLA — Vidí všechny nahrané kontrolní sestavy letáků, komunikuje v komentářích a označuje sestavy jako „Zpracováno“.', 'ikona': 'task_alt'},
    'cenopripad_ctenar_letaky':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Kontrolní data letáků', 'nazev': 'Čtenář – Letáková kontrola', 'popis': 'LETÁKOVÁ KONTROLA — Pouze pro čtení: prohlíží všechny nahrané kontrolní sestavy letáků a smí stahovat exporty. NEMŮŽE nahrávat sestavy, psát komentáře ani měnit stav.', 'ikona': 'visibility'},
    'cenopripad_zadatel_letaky_final': {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Kontrolní data letáků', 'nazev': 'Žadatel – Letáková kontrola (Finální)', 'popis': 'LETÁKOVÁ KONTROLA — FINÁLNÍ SESTAVY. Kopie práva „Žadatel – Letáková kontrola“, ale pro finální sestavy: nahrává je, vidí jen své a komunikuje v komentářích. Bez tohoto práva uživatel finální sestavy vůbec nevidí (má jen kontrolní).', 'ikona': 'sell'},
    'cenopripad_office_letaky_final':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Kontrolní data letáků', 'nazev': 'Office – Letáková kontrola (Finální)', 'popis': 'LETÁKOVÁ KONTROLA — FINÁLNÍ SESTAVY. Kopie práva „Office – Letáková kontrola“ pro finální sestavy: vidí všechny, komentuje, označuje „Zpracováno“ a maže. Bez tohoto práva uživatel finální sestavy vůbec nevidí.', 'ikona': 'task_alt'},
    'cenopripad_ctenar_letaky_final':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Kontrolní data letáků', 'nazev': 'Čtenář – Letáková kontrola (Finální)', 'popis': 'LETÁKOVÁ KONTROLA — FINÁLNÍ SESTAVY. Kopie práva „Čtenář – Letáková kontrola“ pro finální sestavy: pouze čtení a stahování exportů. NEMŮŽE nahrávat, komentovat ani měnit stav.', 'ikona': 'visibility'},
    'cenopripad_zadatel_oprava': {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Oprava cen', 'nazev': 'Žadatel – Oprava cen', 'popis': 'OPRAVA CEN – podává žádosti o opravu ceny (vzorový formulář Oprava_ceny_2026.xlsx), vidí jen své žádosti a jejich stavy, opravuje a znovu odesílá vrácené žádosti nebo trvá na svém a vrací je zpět na Office. Může vlastní nevyřízenou žádost stornovat.', 'ikona': 'sell'},
    'cenopripad_oprava_bez_pcstd': {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Oprava cen', 'nazev': 'Žadatel – Oprava cen bez standardní PC', 'popis': 'MIMOŘÁDNÉ PRÁVO k Oprava cen: ve formuláři nemusí vyplnit sloupec G „Standartní prodejní cena“ — žádost projde i s prázdným sloupcem. V detailu případu se pak u takových řádků zobrazí „—“ místo ceny a nedopočítá se % ani absolutní odchylka.', 'ikona': 'money_off'},
    'cenopripad_office_oprava':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Oprava cen', 'nazev': 'Office – Oprava cen', 'popis': 'OPRAVA CEN – vidí všechny žádosti, zpracuje je, vrací žadateli k opravě, uzavírá je nebo postupuje správci k odsouhlasení; může je stornovat s uvedením důvodu. Dostává e-maily a notifikace o nových žádostech.', 'ikona': 'task_alt'},
    'cenopripad_ctenar_nc':      {'kategorie': 'Modul Cenopřípad', 'podskupina': 'NC OVOZEL', 'nazev': 'Čtenář – NC OVOZEL', 'popis': 'NC OVOZEL — Pouze pro čtení: vidí všechny žádosti o změnu nákupní ceny (import cen i delist), jejich tabulky a průběh. NEMŮŽE zakládat žádosti, vracet je k opravě, měnit stav, exportovat do skladu ani mazat.', 'ikona': 'visibility'},
    'cenopripad_spravce_nakup':  {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Případy (nákup/obchod)', 'nazev': 'Správce nákup',  'popis': 'Plná správa nákupních (letákových) dlaždic — vidí vše včetně OP a marží v %, schvaluje druhou kontrolu. Vidí jen dlaždice leták/sklad6/mimoleták/paima.', 'ikona': 'admin_panel_settings'},
    ...
    'cenopripad_vkladatel':      {'kategorie': 'Modul Cenopřípad', 'podskupina': 'Import dat', 'nazev': 'Vkladatel dat', 'popis': 'Smí pouze nahrávat data (dlaždice „Import dat“ — OP + DATA_POROVNANI). Nevidí žádné případy, OP ani marže v %.', 'ikona': 'upload'},
